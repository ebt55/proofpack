...
| **Pricing (hypothesis)** | **$3 per reviewed application** (usage-based, no seat minimum) *or* **$149 per reviewer seat / month** for high-volume agencies. Both are ~25–40× the measured model cost, leaving room for hosting, support and a browser-capture service. |
| **Cost to serve** | Measured $0.07–$0.19 of Gemini per review (three committed runs); browser compute is negligible; the marginal cost of a review is well under $0.50 all-in. |
...
|---|---|---|
| Applications reviewed; % of website items evidenced automatically (target ≥ 70%) | Reviewer minutes per application: 20–40 → < 5 | Pilot time studies; the tool's own `run.log` and cost lines |
...
- **Expenses:** Gemini API usage only (under $5 for development and the committed runs) — 100%
